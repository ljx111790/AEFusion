...
def run_demo(nest_model, infrared_path, visible_path, output_path_root, index, f_type):
	...
	if c is 1:
		...
		# decoder
		img_fusion_list = nest_model.decoder_eval(f,tp='fuse')
	else:
		# fusion each block
		img_fusion_blocks = []
		for i in range(c):
...
############################ multi outputs ##############################################
	...
	for img_fusion in img_fusion_list:
		file_name = 'AEFusion' + '_' + str(index)  +'_'+ str(output_count) + '_' + f_type + '.png'
...
def main():
	...
	fusion_type = ['attention_avg', 'attention_nuclear']
